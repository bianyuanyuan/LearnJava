...
class QuickSort {
    //received the index of the middle element
    ...
    //recursion call the method
    public static void quickSort(int values[], int low, int high) {
        if (low < high) {
            int middle = getKey(values, low, high);
...
public class TestQuickSort {//test

    public static void main(String[] args) {
        Random rand = new Random(47);
        int arr[] = new int[1000];// rand.nextInt(10000)
        ...
        //QuickSort.nonRecQuickSort(arr);//test the non recursion quick sort
